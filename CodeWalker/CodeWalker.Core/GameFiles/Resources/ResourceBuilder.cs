...
namespace CodeWalker.GameFiles
{
    public class ResourceBuilder
    {
        ...
        public static byte[] Build(ResourceFileBase fileBase, int version, bool compress = true, bool gen9 = false)
        {
            ...
            if (gen9)
            {
                //gen9 resources need size-aware packing since block sizes differ from legacy (BlockLength_Gen9)
            ...
            {
                //use the simpler legacy packing strategy here - the improved AssignPositions2 packing
                //caused crashes for non-gen9 resources, so keep the original behaviour for that path.
